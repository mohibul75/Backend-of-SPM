...
def compare():
    ...
    for i in range(len(data['Category'])):
        ...
        dict_data["Yesterdays_percentage"] = get_yesterday_value_percentage(
            get_today_value_percentage(data['Value'][i], total_todays_value), data['YValue'][i], data['Value'][i])
        dict.append(dict_data)
...
def gainer_loser():
    ...
    for i in range(len(data['Category'])):
        ...
        dict_data["Winner_percentage"] = get_gainer_loser_percentage(data['Winner'][i], data['Loser'][i],
                                                                     data['Neutral'][i])
        dict_data["Neutral"] = data['Neutral'][i]
        dict_data["Neutral_percentage"] = get_gainer_loser_percentage(data['Neutral'][i], data['Winner'][i],
                                                                      data['Loser'][i])
        dict_data["Loser"] = data['Loser'][i]
        dict_data["Loser_percentage"] = get_gainer_loser_percentage(data['Loser'][i], data['Winner'][i],
                                                                    data['Neutral'][i])
        dict.append(dict_data)
